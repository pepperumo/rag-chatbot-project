...
async def fetch_last_message_metadata(supabase: Client, session_id: str) -> Optional[Dict]:
    """
    Fetch metadata from last message to check approval state.
    
    Args:
        supabase: Supabase client
        session_id: Session ID to fetch last message for
        
    Returns:
        Optional[Dict]: Metadata from the last message, or None if no messages exist
    """
    try:
        response = supabase.table("messages") \
            .select("message") \
        ...
        if response.data and len(response.data) > 0:
            message = response.data[0].get("message", {})
            return message.get("data", {})
        return None
    except Exception as e:
        print(f"Error fetching last message metadata: {str(e)}")
        return None
